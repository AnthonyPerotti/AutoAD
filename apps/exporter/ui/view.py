...
from apps.exporter.renderer import ExporterManager

class ExporterView(ctk.CTkFrame):
    def __init__(self, parent, hub):
        ...
        self.btn_add_videos = ctk.CTkButton(
            self.input_panel, text="➕ Add Final Videos", font=("Segoe UI", 13, "bold"),
        ...
        self.lbl_presets = ctk.CTkLabel(self.options_panel, text="Select Platforms to Export:", font=("Segoe UI", 14, "bold"), text_color=COLORS["text_main"])
        self.lbl_presets.pack(anchor="w", padx=20, pady=(15, 10))

        self.var_tiktok = ctk.BooleanVar(value=True)
        self.chk_tiktok = ctk.CTkCheckBox(self.options_panel, text="TikTok (1080x1920, 8M max bitrate)", variable=self.var_tiktok, font=("Segoe UI", 12))
        self.chk_tiktok.pack(anchor="w", padx=20, pady=5)

        self.var_shorts = ctk.BooleanVar(value=False)
        self.chk_shorts = ctk.CTkCheckBox(self.options_panel, text="YouTube Shorts (1080x1920, High Quality)", variable=self.var_shorts, font=("Segoe UI", 12))
        self.chk_shorts.pack(anchor="w", padx=20, pady=5)

        self.var_reels = ctk.BooleanVar(value=False)
        self.chk_reels = ctk.CTkCheckBox(self.options_panel, text="Instagram Reels (1080x1920, 30fps, 8M)", variable=self.var_reels, font=("Segoe UI", 12))
        self.chk_reels.pack(anchor="w", padx=20, pady=5)

        self.var_youtube = ctk.BooleanVar(value=False)
        self.chk_youtube = ctk.CTkCheckBox(self.options_panel, text="YouTube (1080p, Max Quality)", variable=self.var_youtube, font=("Segoe UI", 12))
        self.chk_youtube.pack(anchor="w", padx=20, pady=(5, 20))
        ...
        self.btn_run = ctk.CTkButton(
            row_bottom, text="▶ Export to Socials", font=("Segoe UI", 13, "bold"),
    ...
    def on_finish(self, stopped, errored):
        self.btn_run.configure(
            text=self.lang.get("start_exporter", "▶ Export to Socials"),
        ...
        if stopped:
            ...
            self.log(self.lang.get("done", "Done! Processed batches for selected platforms."))
            if errored:
    ...
    def toggle_run(self):
        if self.exporter_manager.is_running:
        ...
        if not selected_presets:
            messagebox.showwarning("Warning", self.lang.get("exp_no_preset", "Select at least one platform."))
            return
        ...
        self.exporter_manager.process_batch(
    ...
    def update_language(self, lang):
        ...
        if not self.exporter_manager.is_running:
            self.btn_run.configure(text=lang.get("start_exporter", "▶ Export to Socials"))
        else:
            self.btn_run.configure(text=lang.get("cancel", "⏹ Cancel"))
            
        self.lbl_presets.configure(text=lang.get("exp_select", "Select Platforms to Export:"))
