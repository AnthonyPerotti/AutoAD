...
class CTAPanel(SectionCard):
    def __init__(self, parent, on_select_folder, on_clear_folder=None):
        super().__init__(
            ...
            color_key="cta", 
            icon="🎯", 
            subtitle="Selecione a pasta com os vídeos de CTA"
        )
        self.cta_path = ""
    ...
    def open_folder_explorer(self):
        import os
        if self.cta_path and os.path.exists(self.cta_path):
            try: os.startfile(self.cta_path)
            except: pass
        else:
            self.select_folder()
    ...
    def set_path(self, path):
        ...
        if path:
            ...
            self.status_dot.configure(text="●", text_color=COLORS["cta"])
        else:
    ...
    def update_language(self, lang):
        self.lang = lang
        self.set_title(lang["ctas"])
        self.set_subtitle(lang["cta_subtitle"])
