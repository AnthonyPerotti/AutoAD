...
from apps.assembler.render_job import RenderJob
...
class RenderManager:
    ...
    def generate(
        ...
        on_job_update,
        on_finish,
        on_ask_overwrite=None
    ):
        
        naming_pattern = "[AD_{num}] {hook}{body_a}{body_b}{body_c}{cta}"
        ...
        def worker():
            ...
            try:
                    ...
                    for cta in ctas:
                        ...
                        for combinacao in itertools.product(*corpos_videos):
                            ...
                            for i, corpo in enumerate(lista[1:-1]):
                                ...
                                raw_corpos_ids.append(corpo_id)
                            ...
                            kwargs = {f'body_{c}': "" for c in "abcdefghijklmnopqrstuvwxyz"}
                            kwargs['hook'] = hook_id
                            kwargs['cta'] = cta_id
                            kwargs['num'] = str(len(jobs) + 1).zfill(3)
                            kwargs['body'] = '_'.join(corpos_formatados)
                            
                            for j, cid in enumerate(raw_corpos_ids):
                                if j < 26:
                                    kwargs[f'body_{chr(97+j)}'] = cid

                            try:
                                nome_final = naming_pattern.format(**kwargs)
                            except Exception:
                                nome_final = f"H{hook_id}_{'_'.join(corpos_formatados)}_C{cta_id}"
                                
                            if not nome_final.lower().endswith(".mp4"):
                                nome_final += ".mp4"
                            ...
                            jobs.append(job)
                            # We don't call on_job_update here yet, we'll do it after filtering
                ...
                # Filter for overwrite behavior
                existing_jobs = [job for job in jobs if os.path.exists(job.output)]
                
                if existing_jobs and overwrite_behavior == "Ask" and on_ask_overwrite:
                    # Pause and ask main thread
                    event = threading.Event()
                    user_response = {"overwrite": False}
                    
                    def ask_callback():
                        resp = on_ask_overwrite(len(existing_jobs))
                        user_response["overwrite"] = resp
                        event.set()
                        
                    # Call ask_callback in main thread (assumes on_ask_overwrite handles its own thread dispatch or we can just call it if we pass a thread-safe callback)
                    on_ask_overwrite(len(existing_jobs), lambda resp: (user_response.update({"overwrite": resp}), event.set()))
                    event.wait()
                    
                    if not user_response["overwrite"]:
                        on_log("Render cancelado pelo usuário (arquivos existentes).")
                        self.renderizando = False
                        on_finish(True, [])
                        return
                    else:
                ...
                for job in jobs:
                    if os.path.exists(job.output):
                        if overwrite_behavior == "Skip":
                            ...
                            base, ext = os.path.splitext(job.output)
                            counter = 1
                            while os.path.exists(f"{base} ({counter}){ext}"):
                                counter += 1
                            job.output = f"{base} ({counter}){ext}"
                ...
                if total == 0:
                    on_log("Nenhum vídeo para gerar.")
                    self.renderizando = False
                    on_finish(False, jobs)
                    return
